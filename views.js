                        ...
                        <div style={{ position: 'relative' }}>
                          ...
                          {showCityDropdown && (<>
                            ...
                            <div style={{ position: 'absolute', top: '100%', left: '50%', transform: 'translateX(-50%)', marginTop: '4px', background: '#1f2937', borderRadius: '12px', boxShadow: '0 4px 20px rgba(0,0,0,0.4)', zIndex: 50, minWidth: '160px', overflow: 'hidden' }}>
                              {activeCities.sort((a, b) => (a.nameEn || a.name || '').localeCompare(b.nameEn || b.name || '')).map(city => (
                                <button key={city.id}
              ...
              <div className="bg-gradient-to-r from-rose-50 to-orange-50 border-2 border-rose-400 rounded-lg p-2">
                ...
                <div style={{ border: "1px solid #e5e7eb", borderRadius: "8px", overflow: "hidden" }}>
                  {Object.entries(window.BKK.cityRegistry || {})
                    ...
                    .map(([key, reg], i, arr) => {
                      const isActive = reg.active !== false;
                      return (
                        <div key={key} style={{ display: "flex", alignItems: "center", gap: "8px", padding: "8px 10px", borderBottom: i < arr.length - 1 ? "1px solid #f3f4f6" : "none" }}>
                          <span style={{ fontSize: "20px", minWidth: "28px", textAlign: "center" }}>{reg.icon?.startsWith?.("data:") ? "🏙️" : (reg.icon || "🏙️")}</span>
                          <span style={{ flex: 1, fontSize: "13px", fontWeight: "bold", color: "#1f2937" }}>{reg.nameEn || reg.name}</span>
                          <span style={{ fontSize: "10px", color: "#9ca3af", marginRight: "4px" }}>{reg.country}</span>
                          <button
                            onClick={() => {
                              const newActive = !isActive;
                              window.BKK.cityRegistry[key].active = newActive;
                              if (window.BKK.cities[reg.id]) window.BKK.cities[reg.id].active = newActive;
                              setCityActiveStates(prev => ({ ...prev, [reg.id]: newActive }));
                              if (database) {
                                database.ref("settings/cityRegistry/" + key + "/active").set(newActive);
                                database.ref("cities/" + reg.id + "/general/active").set(newActive);
                              }
                              showToast((reg.nameEn || reg.name) + (newActive ? " ✓" : " ✗"), "info");
                            }}
